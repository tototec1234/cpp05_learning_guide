...
基底の getter を呼ぶ。または、チェック自体を基底の `execute()` にまとめ、派生はチェック済みを前提に動作だけ書く。
課題書はどちらでもよい、一方がより elegant とだけ書く。どちらが elegant かは書いていない。基底にまとめるとチェックを忘れない。通常はこちらのほうが elegant である。
