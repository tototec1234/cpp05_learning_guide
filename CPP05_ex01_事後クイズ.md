...
判定が Bureaucrat と Form に二重になる。Form 側の規則を変えたとき、signForm が古いまま残る。  
`beSigned` に投げさせて catch する方が、規則の置き場所が一つ。
...
循環インクルード。インクルードガードがあると、一方のクラスが未完成のまま使われる。  
